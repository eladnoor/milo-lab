...
def get_charge(b, pH, buffer_dict):
...
def buffer_match(buffer_name, buffer_dict, pH):
    ...
    if tmp:
        ...
        if re.findall('\+ koh$', buffer_name):
            charge_conc_pairs += [(1, conc)]
        if re.findall('\+ naoh$', buffer_name):
            charge_conc_pairs += [(1, conc)]
        if b in buffer_dict:
            charge = get_charge(b, pH, buffer_dict)
            charge_conc_pairs.append((charge, conc))
...
################################################################################
# FIRST STEP - create a hash that contains 
# keys (representative names) and values (array of all known aliases for each representative name)
...
################################################################################
# SECOND STEP - each reaction is composed of 'substrates' and 'products'.
...
nist_writer = csv.writer(open('../res/nist_buffered.csv', 'w'))
    ...
    if not row_dict['Ionic strength']:
        ...
        for i in xrange(len(concentrations)):
            ...
            if salt_titles[i] in ['KCl', 'NaCl']:
                ...
                charge = get_charge('phosphate', row_dict['pH'], buffer_dict)
                charge_conc_pairs += [(charge, conc)]
            elif salt_titles[i] in ['Tris']:
                charge = get_charge('tris', row_dict['pH'], buffer_dict)
                charge_conc_pairs += [(charge, conc)]
        ...
        if row_dict['buffer']:
            row_dict['buffer'].replace(' and ', ' + ')
            for b in row_dict['buffer'].split(' + '):
                charge_conc_pairs += buffer_match(b, buffer_dict, row_dict['pH'])
        
        if row_dict['pH']:
            row_dict['pH'] = '%.2g' % row_dict['pH']
        row_dict['Ionic strength'] = "%.2g" % (0.5 * sum([(conc * ch**2) for (ch, conc) in charge_conc_pairs]))
        if Mg_conc > 0:
            row_dict['pMg'] = "%.2g" % -log10(Mg_conc)
        
    nist_writer.writerow([row_dict[k] for k in new_titles])
    
#for i, s in enumerate(sorted(missing_compound_set)):
#    print i, s

...
vp_vec = []
for name in ["peter_2012-04-01"]:
    vp = VictorParser()
    vp.parse_excel("../data/victor/%s.xls" % (name))
    vp_vec.append(vp)

pp = PdfPages('../res/victor/2012-04-01_peter.pdf')
...
for c in xrange(12):
    for r in xrange(8):
        vlegend += [(RowCol2String(r, c), colors[r], 'solid', [(r, c)])]
plots.append(("All plate", (0, t_max), (3e-3, 0.6), 'OD', vlegend))

...
vp_vec = []
for name in ["Elad's OD600_20110217_247"]:
    vp = VictorParser()
    vp.parse_excel("../data/victor/%s.xls" % (name))
    vp_vec.append(vp)

pp = PdfPages('../res/victor/2011-02-20_compare_with_lyconic.pdf')
...
vlegend = []
for c in xrange(9):
    vlegend += [('c%d_up' % c, colors[c], 'solid', [(r, c) for r in xrange(4)])]
    vlegend += [('c%d_down' % c, colors[c], 'dashed', [(r, c) for r in xrange(4, 8)])]
plots.append(('Growth', (0, t_max), (1e-2, 5e-1), 'OD', vlegend))
